...
export async function fetchLatestEmployeeLocation(userId: string, date?: string) {
  try {
    ...
    if (date) {
      ...
      // Get the next day for the upper bound of the range
      const nextDay = new Date(selectedDate);
      nextDay.setDate(nextDay.getDate() + 1);
      const formattedNextDay = nextDay.toISOString().split('T')[0];
      
      // Filter by the date range (inclusive of the selected date, exclusive of the next day)
      query = query
        .gte('captured_at', `${formattedDate}T00:00:00`)
        .lt('captured_at', `${formattedNextDay}T00:00:00`);
    }
...
export async function fetchEmployeeLocationsForDate(userId: string, date: string) {
  try {
    // Convert the date string to a Date object
    const selectedDate = new Date(date);
    // Format the date in YYYY-MM-DD format
    ...
    // Query all locations for the user on the specific date, ordered by capture time
    const { data, error } = await supabase
      .from('employee_locations')
      .select('*')
      .eq('user_id', userId)
    ...
    if (error) {
      console.warn(`Error fetching locations for employee ${userId} on date ${date}:`, error);
      return [];
    }
    
    return data || [];
  } catch (e) {
    console.error(`Exception in fetchEmployeeLocationsForDate for ${userId} on date ${date}:`, e);
...
// New function to fetch attendance logs as fallback when no location data exists
export async function fetchAttendanceLogForDate(userId: string, date: string) {
  try {
    // Convert the date string to a Date object
    const selectedDate = new Date(date);
    // Format the date in YYYY-MM-DD format
    ...
    console.log(`Fetching attendance log for user ${userId} on date ${formattedDate}`);
    
    // Query attendance logs for the user on the specific date
    const { data, error } = await supabase
    ...
    if (error) {
      console.warn(`Error fetching attendance log for user ${userId} on date ${date}:`, error);
      return null;
    }
    
    return data;
  } catch (e) {
    console.error(`Exception in fetchAttendanceLogForDate for ${userId} on date ${date}:`, e);
    return null;
  }
}
