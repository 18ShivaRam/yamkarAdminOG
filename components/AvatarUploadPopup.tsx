...
const AvatarUploadPopup: React.FC<AvatarUploadPopupProps> = ({
  ...
  const notifyParent = (url: string | null) => {
    if (onAvatarUpdate) onAvatarUpdate(url);
    else if (onSuccess) onSuccess(url);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const isSupabaseUrl = urlParts.some(part => part === 'storage' || part === AVATARS_BUCKET);
      
      if (isSupabaseUrl) {
        // Extract the path from the URL
        // Typically in format: https://xxx.supabase.co/storage/v1/object/public/<bucket>/userId/filename
        const pathIndex = urlParts.findIndex(part => part === AVATARS_BUCKET);
        if (pathIndex !== -1) {
          // Path relative to the bucket (exclude the bucket name itself)
          const avatarPath = urlParts.slice(pathIndex + 1).join('/');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // IMPORTANT: Path must be relative to the bucket root (do NOT prefix with bucket name)
      ...
      notifyParent(urlData.publicUrl);
